...
WITH
    ...
    (
        ...
        FROM
            Sales.Orders o INNER JOIN Sales.Products p ON o.ProductID = p.ProductID
...
FROM
    ...
    FROM
        CTE_Highest_Order_Product
    WHERE rn = 1) hop ON ts.CustomerID = hop.CustomerID
    LEFT JOIN (SELECT
        CustomerID,
        Category
    FROM
        CTE_Highest_Category
    WHERE rn = 1) hc ON ts.CustomerID = hc.CustomerID
